...
import 'package:flutter_gram/screens/eventscreen/widgets/edit_event/edit_file_container.dart';
import 'package:flutter_gram/screens/eventscreen/widgets/edit_event/edit_image_container.dart';
import 'package:intl/intl.dart';

List<Step> getSteps(
  EditEventProvider provider,
  Size size,
  String imageUrl,
  ...
  return [
    ...
    Step(
      ...
      state: provider.currentStep > 2 ? StepState.complete : StepState.editing,
    ),
    Step(
      ...
      state: provider.currentStep > 3 ? StepState.complete : StepState.editing,
    ),
    Step(
      ...
      state: provider.currentStep > 4 ? StepState.complete : StepState.editing,
    ),
    Step(
      ...
      state: provider.currentStep > 5 ? StepState.complete : StepState.editing,
    ),
    Step(
      title: const Text('Add Poster'),
      content: provider.isImageChanged
          ? EditFileContainer(
              size: size,
              provider: provider,
            )
      ...
      state: provider.currentStep > 6 ? StepState.complete : StepState.editing,
    ),
  ];
}
